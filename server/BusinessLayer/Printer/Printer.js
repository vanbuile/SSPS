...
const AddNewPrinter = async (req, res) => {
    ...
    try {
        let result = await addPrinter(name, brand, model, facility, building,floor, paper, day, description, status)
    }
    catch (e) {
        console.log(e)
...
const EditOldPrinter = async (req, res) => {
    ...
    let description = req.body.description
    let status = req.body.status
    let rs = await editPrinter(id, name, brand, model, facility, building, floor, paper, day, description, status)
}
